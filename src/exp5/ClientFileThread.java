		...
		try {
			...
			while(true) {
				...
				if(result == 0){
					...
					out.flush();
					String thisPath = "E:\\IntelliJ IDEA 2020 Workspace\\multichat-system-master\\src\\file\\";
					File userFile = new File( thisPath + userName);
					if(!userFile.exists()) {  // 新建当前用户的文件夹
						userFile.mkdir();
					}
					File file = new File(thisPath + userName + "\\"+ textName);
